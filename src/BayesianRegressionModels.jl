...
#   - 2. Allow for estimating group memberships of random effect levels inside the Turing model
#   - 4. Make example with multi-step Turing model
#   - 5. Make example with categorical predictors
#   - 6. [!] Upgrade update! for predictors:
            # Automatically split up a vector of categories so it can be used for the right places in the design matrix
            # Allow for multiple terms
#   - 7. [!] Upgrade RegressionPredictors
            # Make design matrices be views of an underlying predictor matrix
            # Make there be a matrix with categorical predictors in non-one-hot-encoded format 
# - B. Optimisation
