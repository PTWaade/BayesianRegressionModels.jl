...
## 2. Submodel for evaluating the likelihood of data, parameterising a distribution with predictions, a prior, or pre-specified values ##
@model function regression_likelihood(
    outcomes::Toutcomes,
    predictions::Tpredictions,
    likelihood_info::Tlikelihood,
) where {Toutcomes<:AbstractVector, Tpredictions<:NamedTuple, Tlikelihood<:DistributionLikelihood,}
    ...
    dist_args = Tuple(
        begin
            ...
            if arg isa Tuple{Symbol, Function} # If a symbol has been specified
                ...
                inv_link.(predictions[predictions_label])
